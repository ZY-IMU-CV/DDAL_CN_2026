...
def get_model(args):
    if args.dataset == "ImageNet-LT" :
        ...
        print("=> creating model '{}'".format('resnet152'))
        net = Resnet_LT.resnet152(num_classes=args.num_classes, pretrained_path=args.pretrained_path)
        return net
    else:
...
def main_worker(gpu, args):
    ...
    train_sampler = None
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None),num_workers=args.workers, persistent_workers=True,pin_memory=False, sampler=train_sampler)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False,num_workers=args.workers, persistent_workers=True,pin_memory=False)
    ...
    weighted_train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size,num_workers=args.workers, persistent_workers=True,pin_memory=False,sampler=weighted_sampler)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='cifar100', help="cifar10,cifar100,ImageNet-LT,iNaturelist2018,Places-LT")
    parser.add_argument('--root', type=str, default='data/', help="data/, data/ImageNet, data/places")
    parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet34',choices=('resnet12', 'resnet32','resnet34', 'resnet50', 'resnext50_32x4d','resnet152'))
    ...
    parser.add_argument('--cbfa_weight', default=10, type=int, help='CBFA weight')
    parser.add_argument('--pgsa_weight', default=1, type=float, help='PGSA weight')
    ...
    parser.add_argument('--dir_test_txt', type=str, default='data/data_txt/ImageNet_LT_test.txt', required=False, help='Path to the test data txt file')
    parser.add_argument('--dir_train_txt', type=str, default='data/data_txt/ImageNet_LT_train.txt', required=False, help='Path to the training data txt file')
    parser.add_argument('--pretrained_path', type=str, default='model/resnet152.pth', required=False, help='Path to the pretrained model file')
