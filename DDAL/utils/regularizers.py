...
class MaxNorm_via_PGD():
    ...
    def setPerLayerThresh(self, model):
        ...
        if isinstance(model, torch.nn.DataParallel):
            layers = [model.module.fc_cb.weight, model.module.fc_cb.bias]
        else:
            layers = [model.fc_cb.weight, model.fc_cb.bias] 

        for i, curLayer in enumerate(layers):
        # for curLayer in [model.fc_cb.weight, model.fc_cb.bias]: #here we only apply MaxNorm over the last two layers
    ...
    def PGD(self, model):
        ...
        if isinstance(model, torch.nn.DataParallel):
            ...
            target_model = model
        for i, curLayer in enumerate([target_model.fc_cb.weight, target_model.fc_cb.bias]): #here we only apply MaxNorm over the last two layers
            curparam = curLayer.data
...
class Normalizer(): 
    ...
    def apply_on(self, model): #this method applies tau-normalization on the classifier layer
        ...
        for curLayer in [model.fc_cb.weight]: #change to last layer: Done
            curparam = curLayer.data
